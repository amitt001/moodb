...
// Update an alias for Set
func (s *server) Update(ctx context.Context, in *pb.SetRequest) (*pb.SetResponse, error) {
	log.Printf("UPDATE: %s", in.Key)
	var respMsg string
	value, err := s.db.Update(in.Key, in.Value)
	if err != nil {
		respMsg = err.Error()
	}
	return &pb.SetResponse{Message: value, RespMsg: respMsg, StatusCode: 201}, nil
...
func Run() {
	...
	s := grpc.NewServer()
	pb.RegisterMdbServer(s, &server{newDb(dbName)})
